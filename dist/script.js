...
function showRecipe(i) {
	if (done) 
	{
		doc.getElementsByClassName('dif')[i].style.display="flex";
		doc.getElementsByClassName('cos')[i].style.display="flex";
		doc.getElementsByClassName('rec')[i].style.display="flex";
		done = false;
	}
	else{
		doc.getElementsByClassName('dif')[i].style.display="none";
...
addUserToList = (data) => {
    ...
    div.innerHTML = `
                        <div class="profile-info">
                            <h4>${data.name}</h4> by ${data.by}
                        </div>
                        <div class="difficulty" style="display:none">
                            <span class="difficulty-name">${data.dif}</span>
                        </div>
